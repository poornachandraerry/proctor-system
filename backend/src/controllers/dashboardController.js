...
async function getDashboard(req, res) {
  try {
    ...
    if (role === 'admin') {
      ...
      // ── STUDENT ────────────────────────────────────────
      // FIX (Issue #4): Previously this matched ANY published exam with
      // access_type='open' regardless of organisation, which leaked other
      // orgs'/examiners' exams to every student. Now a student only sees:
      //   (a) exams they are explicitly enrolled in (via exam_enrollments
      //       or the email/domain whitelist self-registration flow), OR
      //   (b) "open" exams created within their OWN organisation.
      // A student with no org_id (e.g. legacy/demo accounts) sees nothing
      // until properly enrolled or assigned an org — this is intentional.
      const [enrolled, completed, upcoming, completedSessions] = await Promise.all([
        ...
        query(`
          SELECT DISTINCT e.id, e.title, e.duration_minutes, e.start_time, e.status, e.description
          FROM exams e
